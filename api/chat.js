...
- Cap only on DISPOSITIVE uncertainty, not on secondary unknowns. An open item lowers the tier only if it could actually deflate the claim on the merits — a genuine factual dispute about the claimant's own conduct, or a core element that is truly unconfirmed. Procedural or evidentiary nice-to-haves do NOT cap the tier: exactly whom a complaint was routed to when reporting to any of them is protected; exact headcount when a state statute already covers the employer regardless; whether a document is physically in hand yet. When you rate below HIGH, identify the specific merit-deflating uncertainty that drove the cap — and if the only things still open are secondary items like those, they are not what is holding the case back, so do not cap on them.
